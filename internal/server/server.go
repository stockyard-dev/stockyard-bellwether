...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/monitors", s.list)
	s.mux.HandleFunc("POST /api/monitors", s.create)
	s.mux.HandleFunc("GET /api/monitors/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/monitors/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-bellwether", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"monitors": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Monitor{} }
	wj(w, map[string]any{"monitors": items, "count": len(items)})
}
